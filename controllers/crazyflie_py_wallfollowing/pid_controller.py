...
class pid_velocity_fixed_height_controller():
    def __init__(self):
        ...
        self.last_time = 0.0
        # Default gains (Crazyflie specific) - Move these to params for F450 later
    ...
    def pid(self, dt, desired_vx, desired_vy, desired_yaw_rate, desired_altitude, actual_roll, actual_pitch, actual_yaw_rate,
        ...
        desired_pitch = self.gains["kp_vel_xy"] * np.clip(vx_error, -1, 1) + self.gains["kd_vel_xy"] * vx_deriv
        desired_roll = -self.gains["kp_vel_xy"] * np.clip(vy_error, -1, 1) - self.gains["kd_vel_xy"] * vy_deriv
        ...
        alt_command = self.gains["kp_z"] * alt_error + self.gains["kd_z"] * alt_deriv + \
            self.gains["ki_z"] * np.clip(self.altitude_integrator, -2, 2) + 48
        ...
        roll_command = self.gains["kp_att_rp"] * np.clip(roll_error, -1, 1) + self.gains["kd_att_rp"] * roll_deriv
        pitch_command = -self.gains["kp_att_rp"] * np.clip(pitch_error, -1, 1) - self.gains["kd_att_rp"] * pitch_deriv
        yaw_command = self.gains["kp_att_y"] * np.clip(yaw_rate_error, -1, 1)
